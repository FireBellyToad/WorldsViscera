...
use super::common::{Position, Target, Viewshed};
...
fn try_move_player(delta_x: i32, delta_y: i32, ecs_world: &World) {
    let mut players = ecs_world.query::<(&Player, &mut Position, &mut Target, &mut Viewshed)>();
    let mut map_query = ecs_world.query::<&Map>();
    let (_e, map) = map_query.iter().last().expect("Map is not in hecs::World");

    for (_e, (_p, position, target, viewshed)) in &mut players {
